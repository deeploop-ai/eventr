...
| **AI / Agent** | 一等公民 `llm` / `embed` / `agent` Transform；可插拔 LLM Provider；DAG 原生多 Agent 编排 — 详见 [AI/Agent 指南](docs/ai-agent.md) |
...
LLM 分类 + 路由与 ETL 共用同一 DAG 模型，无需独立编排层：
...
RAG 摄取、Tool Calling Agent 与分阶段路线图见 [AI/Agent 指南](docs/ai-agent.md)。
...
| [`docs/ai-agent.md`](docs/ai-agent.md) | **AI/Agent 指南**（LLM Transform、Provider、多 Agent DAG、路线图） |
