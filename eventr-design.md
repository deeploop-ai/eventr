...
2. **结构对齐 Envelope、字段名保持 eventr** — 采用 Envelope 的 `steps.{name}` 嵌套块布局（`source` / `transform` / `sink`）；字段用 `depends_on`、`decoder`/`encoder`、`type`/`config`、`engine` 等（`type` = 组件实现名，如 `kafka`/`map`）
3. **渐进复杂度** — 线性场景仅 `steps` + `depends_on`；复杂场景加 `edges`（per-edge buffer/delivery，eventr 扩展）
4. **统一 IR** — 两种格式与 K8s CRD（YAML）均汇入同一 `TopologyIR`
5. **substitution 对等** — 两种格式均支持 `${ENV_VAR}` 与 `${config.path}` 引用（见 §8.2）

#### 8.1.2 字段 `kind` 与 `type`

| 字段 | 含义 | 出现位置 |
|------|------|----------|
| **`type`** | 组件实现名（`kafka`、`map`、`http`、`wasm`…） | `steps.*.source/transform/sink` 子块；平坦 `stages[]` |
| **`kind`** | Stage 角色（`source` / `transform` / `sink`） | 仅平坦 `stages[]`（子块写法由块名隐含，不需 `kind`） |

加载器校验：`type` 必须在注册表中存在且与 `kind`（或子块角色）匹配。
...
| `steps.{name}` | `steps.{name}` | step 名即 stage id |
| `steps.{name}.dependencies` | `steps.{name}.depends_on` | `depends_on: [a, b]` |
...
| `steps.{name}.output` | `steps.{name}.sink` | `type`、`encoder`、`batch`、`config`（含该 sink 自有的写入语义配置） |
| `udfs` | `udfs` / `cel.functions` | 可选，v2 评估 |

> Envelope 的 `planner` 块**不映射**为 eventr 配置项；原 planner 语义由目标 **sink 插件的 `config`** 自行表达（各插件字段不同，引擎不做统一抽象）。
>
> Envelope 用户迁移时：对照**块的位置与嵌套关系**，把 `input` 搬进 `source`、`deriver` 搬进 `transform`、`output` 搬进 `sink`。
...
| `.conf` / `.hocon` | HOCON 解析器（实现选型：`github.com/gurkankaymak/hocon` 或同等成熟库，启动期评估） |
...
type PipelineConfig struct {
    ...
    Steps         map[string]StepConfig     // 主写法：steps.{name}，结构对齐 Envelope
    Stages        []StageConfig             // 平坦写法：stages[]（与 Steps 二选一）
    Codecs        []CodecIR
    Edges         []EdgeIR
...
// StepConfig — 单 step 可含 source / transform / sink 子块（Envelope 式合体）
type StepConfig struct {
    StepType   string         // data | loop | decision | task；默认 data（v2.1+）
    DependsOn  []string
    Source     *SourceBlock   // 仅源 step：对标 Envelope input 的位置
    Transform  *TransformBlock
    Sink       *SinkBlock     // 对标 Envelope output 的位置
}
...
| **`stages[]` 平坦列表**（§8.3） | 简单管道、与早期草案兼容 | 每行一个 stage，`kind` + `type` + `depends_on` |

两种形态经加载器展开后生成相同的 `TopologyIR`。

### 8.3 场景一：简单线性（平坦 `stages[]` 写法）
...
### 8.4 场景二：显式 DAG（edges + condition + route）
...
type StageIR struct {
    ID         string
    Kind       StageType          // source / transform / sink；配置键 kind
    Type       string             // kafka / http / map / filter / route / wasm / …；配置键 type
...
配置加载时将语法糖展开为 `TopologyIR`（**YAML 与 HOCON 规则相同**）：

| 配置字段 | 展开结果 |
|----------|---------|
| `depends_on: [a, b]` | 2 条 EdgeIR: `{from: a, to: self}, {from: b, to: self}` |
| `steps.{name}.source` only | StageIR{id=name, kind=source, type=kafka, decoder, config} |
| `steps.{name}.transform`（无 sink） | StageIR{id=name, kind=transform, type=map, ...} |
| `steps.{name}.transform` + `sink` | 展开为 transform stage + sink stage（默认 id=`{name}-sink`）；自动加 edge `{name}→{name}-sink` |
| `stages[]` 平坦项 | 直接映射 StageIR；`depends_on` 展开为 EdgeIR |
| `route: us`（edge 上） | 自动生成 `condition: "metadata['er-route'] == 'us'"` |
| `sink.config.parameter.*` | 剥 `parameter.` 前缀合并入 sink 插件 `config` |

`steps` 合体 step 展开示例：

```
steps.enrich.depends_on = [kafka-in]
steps.enrich.transform  → stage id=enrich,      kind=transform, type=map
steps.enrich.sink       → stage id=enrich-sink, kind=sink, type=kafka
                        → edge enrich → enrich-sink
                        → edges kafka-in → enrich（来自 depends_on）
```
...
.yaml/.yml/.conf/.hocon 文件 / K8s CRD(YAML) / 目录
  ...
  substitution 解析（${ENV}、${path}、${?optional}）
        │
        ▼
  steps 规范化（若使用 steps 块）+ depends_on 展开
        │
        ▼
...
- [ ] Message 模型 + Stage/Edge/Codec 接口定义
- [ ] Topology IR + YAML/HOCON 双格式解析 + steps 规范化 + depends_on 展开 + 校验
